  ...
  return (
    ...
    >
      <div className="text-center md:text-left">
        <h2 className="text-2xl font-bold text-slate-800 mb-2 tracking-tight">Step {getStepNumber(8)}: Inspiration & Extras</h2>
        <p className="text-base text-slate-500 font-medium opacity-90">
          {data.projectType === 'Logo Design' 
            ? "Show us brands you admire or want to compete with."
            : "Show us some websites you like or are like yours."}
        </p>
      </div>

      <div className="space-y-5">
        <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
           <Search className="w-5 h-5 text-blue-600" /> {data.projectType === 'Logo Design' ? "Businesses you admire" : "Any websites like yours?"}
        </h3>
        <p className="text-sm text-slate-500 font-medium">{data.projectType === 'Logo Design' ? "Add links to brands that share your vibe." : "Add links to your competitors or similar businesses."}</p>
           ...
           {data.competitorWebsites.map((c, idx) => (
             <div key={idx} className="flex gap-2">
                <input 
                  ...
                  className="flex-1 p-4 bg-white border-2 border-slate-50 rounded-2xl outline-none focus:border-blue-500 font-medium transition-all text-sm"
                />
