...
export const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 relative overflow-hidden font-sans selection:bg-blue-500 selection:text-white">
      
      {/* Intense Background Glows (Light Mode version - Multiply blend instead of screen, softer opacity) */}
      <div className="absolute top-[-30%] left-[-10%] w-[1000px] h-[1000px] bg-blue-300 rounded-full mix-blend-multiply filter blur-[150px] opacity-40 animate-pulse"></div>
      <div className="absolute top-[20%] right-[-20%] w-[800px] h-[800px] bg-purple-300 rounded-full mix-blend-multiply filter blur-[150px] opacity-30 animate-pulse animation-delay-2000"></div>
      <div className="absolute bottom-[-20%] left-[20%] w-[800px] h-[800px] bg-emerald-200 rounded-full mix-blend-multiply filter blur-[150px] opacity-40 animate-pulse animation-delay-4000"></div>

      {/* Grid Pattern overlay (dark grid dots on light background) */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px)] bg-[size:64px_64px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_0%,#000_70%,transparent_100%)] pointer-events-none"></div>

      {/* Nav */}
      <nav className="relative z-20 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto border-b border-slate-200">
        ...
        <div className="flex items-center gap-8">
          <button onClick={() => navigate('/login')} className="text-sm font-bold text-slate-600 hover:text-blue-600 transition-colors hidden sm:block">Developer Portal</button>
          <button onClick={() => navigate('/login')} className="text-sm font-bold text-slate-600 hover:text-purple-600 transition-colors hidden sm:block">Admin Engine</button>
          <button 
            onClick={() => navigate('/login')}
            className="px-6 py-2.5 bg-slate-900 text-white rounded-full text-sm font-bold shadow-lg shadow-slate-900/20 hover:shadow-xl hover:scale-105 transition-all"
          >
            Sign In
          </button>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center pt-32 pb-40 px-6 max-w-6xl mx-auto text-center">
        ...
        >
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 text-sm font-bold mb-10 shadow-sm backdrop-blur-md">
            <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs font-black uppercase tracking-wider mr-2">New</span>
            The Ultimate Intake Architecture
          </div>

          <h1 className="text-6xl md:text-8xl lg:text-[7rem] font-black text-slate-900 mb-8 tracking-tighter leading-[1]">
      ...
      {/* What it helps us do */}
      <section className="relative z-10 py-32 bg-white/60 backdrop-blur-3xl border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight mb-6">Why ProjectHub?</h2>
            <p className="text-xl text-slate-500 max-w-2xl mx-auto font-medium">We built this platform to obliterate the friction between a client's dream and a developer's execution context.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {benefits.map((b, idx) => (
              <motion.div 
                ...
                className="bg-white border border-slate-100 p-10 rounded-[2rem] hover:bg-slate-50 transition-all hover:-translate-y-2 group shadow-xl shadow-slate-200/50"
              >
                <div className={`w-16 h-16 rounded-2xl ${b.color} flex items-center justify-center mb-8 shadow-lg shadow-${b.color.replace('bg-', '')}/30 group-hover:scale-110 transition-transform duration-500`}>
                  {b.icon}
                </div>
                <h3 className="text-2xl font-black text-slate-900 mb-4">{b.title}</h3>
                <p className="text-slate-600 leading-relaxed font-medium text-lg">{b.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 py-32 overflow-hidden bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-24">
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight mb-6 relative inline-block">
              How It Works
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, idx) => (
              <motion.div 
                ...
                className="relative"
              >
                <div className="text-8xl font-black text-slate-200 mb-6 -ml-4 select-none">{step.num}</div>
      ...
      {/* CTA Footer */}
      <footer className="relative z-10 border-t border-slate-200 bg-white overflow-hidden">
        ...
        <div className="border-t border-slate-100 py-8 text-center text-slate-400 font-medium text-sm bg-slate-50">
