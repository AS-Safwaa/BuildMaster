...
import { 
  LayoutDashboard, Settings, LogOut, Plus, Edit2, Trash2, X, 
  CheckSquare, ClipboardList, UserCircle, Activity, 
  ArrowUpRight, Database, RotateCcw
} from 'lucide-react';
      ...
      <main className="flex-1 flex flex-col relative w-full overflow-hidden">
        ...
        <div className="flex-1 p-10 overflow-auto pb-40 w-full bg-[#f8fafc] custom-scrollbar">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="h-full max-w-7xl mx-auto">
                      ...
                      {overviewData.recentProjects.map((p: any) => (
                          <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                            ...
                            <td className="px-8 py-5 text-center">
                               <button onClick={() => openProject(p.id)} className="text-blue-600 hover:text-blue-800 font-bold text-[10px] uppercase tracking-widest flex items-center gap-1.5 transition-all mx-auto">
                                  <ClipboardList className="w-3.5 h-3.5" /> Workspace
                               </button>
                            </td>
                          </tr>
                      ))}
                  ...
                  <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Developer</th>
                      ...
                      {projects.map((p: any) => (
                        <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="px-8 py-5">
                              <span className="block font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{p.businessName}</span>
                          ...
                          <td className="px-8 py-5">
                             <select 
                               className="block w-full bg-white border border-slate-200 text-slate-700 rounded-lg px-4 py-2 text-xs font-semibold focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            {/* Profile Tab and others remain consistent... */}
            {activeTab === 'profile' && (
              <div className="max-w-xl mx-auto py-10">
                 <h1 className="text-3xl font-bold text-slate-900 mb-2 tracking-tight text-center">Admin Profile CMS</h1>
                 <form onSubmit={handleUpdateProfile} className="bg-white mt-10 p-8 rounded-2xl shadow-sm border border-slate-200 space-y-5">
                    <input type="text" value={profileName} onChange={e=>setProfileName(e.target.value)} placeholder="Full Name" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold outline-none" />
                    <input type="email" value={profileEmail} onChange={e=>setProfileEmail(e.target.value)} placeholder="Email" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold outline-none" />
